...
public class JumpTree {
	...
	if (!Utilities.isArrayEmpty(checker.getJumpablePlaces(x, y, null))) {
		...
		Checker piece = (Checker) Chess_Checkers.client.getBoard()[places[i].getMidPoint().x][places[i]
			.getMidPoint().y];
	...
	for (int i = 0; i < length; i++) {
		...
		if (Utilities.isArrayEmpty(places)) {
			...
			Checker piece = (Checker) Chess_Checkers.client.getBoard()[places[n].getMidPoint().x][places[n]
				.getMidPoint().y];
			tree[tree.length - places.length + n].previousUIDs.add(piece.UID);
		    }
		    tree[i].addMidJump(places[0]);

		    Checker piece = (Checker) Chess_Checkers.client.getBoard()[places[0].getMidPoint().x][places[0]
			    .getMidPoint().y];
